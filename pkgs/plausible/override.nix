...
let
  ...
  # its Makefile git-clones lexbor at build time (which sandbox forbids)
  # pre-seed commit in lazy_html's mix.exs (@lexbor_git_sha) so the clone target is skipped
  # and force a from-source build in preBuild below
...
beamPackages.mixRelease rec {
  ...
  preBuild = ''
    ...
    # tracker must be writable since 3.1.0: compile.js emits npm_package/plausible.js
