...
local function find_port(node_bid, name, direction)
  for port in port_om:iterate() do
    if tonumber(port.properties["node.id"]) == node_bid
       and port.properties["port.name"] == name
       and port.properties["port.direction"] == direction then
      return port
    end
  end
  return nil
...
local function link_exists(src_bid, dst_bid)
  for link in link_om:iterate() do
    if tonumber(link.properties["link.output.port"]) == src_bid
       and tonumber(link.properties["link.input.port"]) == dst_bid then
      return true
    end
  end
  return false
...
local function destroy_other_links_from(src_node_bid, keep_dst_node_bid)
  for link in link_om:iterate() do
    if tonumber(link.properties["link.output.node"]) == src_node_bid
       and tonumber(link.properties["link.input.node"]) ~= keep_dst_node_bid then
...
local function ensure_link(src_port, dst_port)
  if not src_port or not dst_port then return end
  if link_exists(src_port["bound-id"], dst_port["bound-id"]) then return end
  Link("link-factory", {
