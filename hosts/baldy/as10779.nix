...
in
{
  ...
  services.as10779 = {
    ...
    local = {
      ...
      interface = {
        local = lib.blueprint.hosts.baldy.ipam.interface;
        primary = lib.blueprint.hosts.baldy.interface;
      };
      ipv4.addresses = [
        "${lib.blueprint.hosts.baldy.ipam.ipv4}/32" # unicast
        "23.161.104.17/32" # personal site anycast
      ];
      ipv6.addresses = [
        "${lib.blueprint.hosts.baldy.ipam.ipv6}/128" # unicast
    ...
    router = {
      ...
      source = { inherit (lib.blueprint.hosts.baldy) ipv4 ipv6; };
      static =
        let
          option = "reject";
        in
